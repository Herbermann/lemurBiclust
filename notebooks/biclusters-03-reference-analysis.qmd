---
title: "Biclusters 03 -- reference analysis in edgeR"
author: "Tim"
date: last-modified
format:
  html:
...
```{r}
# Speed developed w/ Claude -- review.
...
if (file.exists(file.path(cache_dir, "pseudobulk_gsea_results.rds"))){
  results <- readRDS(file.path(cache_dir, "pseudobulk_gsea_results.rds"))
} else {
  ...
  saveRDS(results, file.path(cache_dir, "pseudobulk_gsea_results.rds"))
...
if (file.exists(file.path(cache_dir, "pseudobulk_gsea_table.rds"))){
  sig_table <- readRDS(file.path(cache_dir, "pseudobulk_gsea_table.rds"))
} else {
  sig_table <- do.call(
  rbind,
  lapply(results, function(x) {
  ...
  saveRDS(sig_table, file.path(cache_dir, "pseudobulk_gsea_table.rds"))
}
## ------------------------------------------------------------
...
#  saveRDS(results, file.path(cache_dir, "pseudobulk_gsea_results.rds"))
#}
#if (!file.exists(file.path(cache_dir, "nes_mat_cluster.rds"))){
#  saveRDS(nes_mat_cluster, file.path(cache_dir, "nes_mat_cluster.rds"))
#}
#saveRDS(padj_mat_cluster, file.path(cache_dir, "padj_mat_cluster.rds"))
